...
A small Go CLI that tests LLM API compatibility against OpenAI-compatible and
Anthropic-compatible endpoints. It speaks the raw HTTP wire format — no SDK —
so it exercises what a compatible server actually accepts and returns.
...
internal/cmd/cmd.go          # cobra CLI: run / list / --api / --model / --out
...
internal/runner/             # Case interface + result reporting
