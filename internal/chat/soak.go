...
// SoakCase runs one short streamed chat completion per soak turn and
...
func (c *SoakCase) ID() string   { return "chat:soak" }
func (c *SoakCase) Desc() string { return "POST /v1/chat/completions stream stability over a long run" }

// RunTurn runs one streamed request and returns its classification. The
// stream must end with a completion marker ([DONE] or a finish_reason chunk):
// EOF before either is a mid-stream cut. When no stream event arrives for
// stall, the request is torn down and the turn is classified as stalled.
func (c *SoakCase) RunTurn(ctx context.Context, model string, long bool, stall time.Duration) *registry.SoakTurn {
	...
	defer cancelReq()
	resp, err := c.client.oc.Do(reqCtx, "/chat/completions", body)
	if err != nil {
	...
	// Read the stream to its completion marker under a no-data watchdog.
	...
	for {
		...
		if err != nil {
			...
			if stalled.Load() {
				...
				tr.Err = fmt.Errorf("stream cut before [DONE] or finish_reason: %w", err)
			}
			tr.Total = time.Since(start)
			return tr
		}
	...
	if !complete {
		tr.Class = registry.SoakDropped
		tr.Err = fmt.Errorf("stream ended without [DONE] or finish_reason")
	} else {
